...
def run_tests():
    ...
    try:
        ...
        for i in range(5):
            ...
            keep_index = i
            keep_id = transforma(
                data["test_03_{:02}D1".format(i)], data["test_03_{:02}D2".format(i)]
            )
            assert (
            ...
            assert (
                keep_id == data["test_03_{:02}A".format(i)]
            ), "Falhou na comparação de resultados do test_03_{:02}".format(i)
            print("OK")
            test_results += 1
    except IOError as e:
        print("FALHOU!")
        ...
        print("################################################")
        print(
            "Para o matriz M0: {0}".format(data["test_03_{:02}D1".format(keep_index)])
        )
        print(
            "Com transformações: {0}".format(data["test_03_{:02}D2".format(keep_index)])
        )
        print("################################################")
        print(
            "O resultado esperado era: {0}".format(
                data["test_03_{:02}A".format(keep_index)]
            )
        )
